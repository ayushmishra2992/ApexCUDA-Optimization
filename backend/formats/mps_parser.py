    ...
    def parse(self, filepath):
        ...
        with open(filepath, "r") as f:
            for line_number, raw_line in enumerate(f, start=1):
                ...
                if first_token == "BOUNDS":
                    section = "BOUNDS"
                    continue

                if first_token == "OBJSENSE":
                    section = "OBJSENSE"
                    continue

                if first_token == "ENDATA":
                    saw_endata = True
                    break
                ...
                # ---------------------------------------------------------
                    ...
                    #
                    # Process only complete row/value pairs. Some benchmark
                    # MPS files contain a dangling final token, so do not let
                    # that incomplete pair prevent the rest of the model
                    # from being parsed.
                    #
